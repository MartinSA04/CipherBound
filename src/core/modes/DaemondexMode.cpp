...
{
    int totalEntries = ctx.pokedex.speciesCount() - 1; // exclude dummy at index 0
    if (totalEntries <= 0)
        return;
...
void DaemondexMode::drawList(GameContext &ctx)
{
    ...
    const Player &player = ctx.world.getPlayer();
    int totalEntries = ctx.pokedex.speciesCount() - 1;
    ...
    for (int i = 0; i < VISIBLE_ROWS && (scrollOffset + i) < totalEntries; ++i)
    {
        ...
        if (isSelected)
        {
            renderer.drawFilledRect(listX - 2 * scale, rowY, WINDOW_WIDTH - 12 * scale, rowHeight - 2, TDT4102::Color{180, 200, 240});
        }
    ...
    if (totalEntries > VISIBLE_ROWS)
    {
        ...
        int barTop = listY;
        int barFullH = VISIBLE_ROWS * rowHeight;
        int thumbH = std::max(8, barFullH * VISIBLE_ROWS / totalEntries);
        int thumbY = barTop + (barFullH - thumbH) * scrollOffset / (totalEntries - VISIBLE_ROWS);
...
void DaemondexMode::drawDetail(GameContext &ctx)
{
    ...
    if (player.hasCaught(speciesId))
    {
        ...
        font.drawText(renderer, "SPD: " + std::to_string(sp.baseStats.specialDefense), col2X, statsY + 14 * scale, scale);
