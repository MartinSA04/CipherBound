...
void DaemondexMode::drawList(GameContext &ctx) {
    ...
    renderer.drawFilledRect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT,
                            TDT4102::Color{230, 235, 245});

    // Title bar
    renderer.drawFilledRect(0, 0, WINDOW_WIDTH, 14 * scale,
                            TDT4102::Color{70, 90, 140});
    font.drawText(renderer, "DAEMONDEX", 8 * scale, 1 * scale, scale);

    // Seen/caught counts
    std::string countsText =
        "Seen: " + std::to_string(player.seenCount()) +
        "  Caught: " + std::to_string(player.caughtCount());
    int countsW = font.getTextWidth(countsText, scale);
    font.drawText(renderer, countsText, WINDOW_WIDTH - countsW - 8 * scale,
                  1 * scale, scale);
    ...
    for (int i = 0; i < VISIBLE_ROWS && (scrollOffset + i) < totalEntries;
         ++i) {
        ...
        if (isSelected) {
            renderer.drawFilledRect(listX - 2 * scale, rowY,
                                    WINDOW_WIDTH - 12 * scale, rowHeight - 2,
        ...
        while (numStr.size() < 3)
            numStr = "0" + numStr;
        font.drawText(renderer, numStr, listX + 2 * scale, rowY + 2 * scale,
                      scale);
        ...
        // Selection arrow
        if (isSelected) {
            ctx.ui.drawSelectionArrow(listX - 2 * scale, rowY + 4 * scale,
                                      scale);
        }
    }
    ...
    if (totalEntries > VISIBLE_ROWS) {
        ...
        int thumbH = std::max(8, barFullH * static_cast<int>(VISIBLE_ROWS) /
                                     static_cast<int>(totalEntries));
        int thumbY = barTop + (barFullH - thumbH) *
                                  static_cast<int>(scrollOffset) /
                                  static_cast<int>(totalEntries - VISIBLE_ROWS);

        renderer.drawFilledRect(barX, barTop, 2 * scale, barFullH,
                                TDT4102::Color{200, 200, 210});
        renderer.drawFilledRect(barX, thumbY, 2 * scale, thumbH,
...
void DaemondexMode::drawDetail(GameContext &ctx) {
    ...
    // Border frame
    int margin = 6 * scale;
    renderer.drawRect(margin, margin, WINDOW_WIDTH - 2 * margin,
                      WINDOW_HEIGHT - 2 * margin, TDT4102::Color::transparent,
                      TDT4102::Color{70, 90, 140});
    ...
    if (renderer.hasTexture(frontId)) {
        renderer.drawSpriteRaw(frontId, spriteX, spriteY, spriteSize,
                               spriteSize);
    }
    ...
    // Types
    std::string type1 =
        StringUtils::capitalize(elementTypeName(sp.primaryType));
    font.drawText(renderer, "Type: " + type1, infoX, infoY, scale);

    if (sp.secondaryType != sp.primaryType) {
        std::string type2 =
            StringUtils::capitalize(elementTypeName(sp.secondaryType));
        font.drawText(renderer, " / " + type2,
                      infoX + font.getTextWidth("Type: " + type1, scale), infoY,
                      scale);
    }
    ...
    if (player.hasCaught(speciesId)) {
        ...
        font.drawText(renderer, "HP:  " + std::to_string(sp.baseStats.hp),
                      infoX, statsY, scale);
        font.drawText(renderer, "ATK: " + std::to_string(sp.baseStats.attack),
                      infoX, statsY + 14 * scale, scale);
        font.drawText(renderer, "DEF: " + std::to_string(sp.baseStats.defense),
                      infoX, statsY + 28 * scale, scale);

        int col2X = infoX + 60 * scale;
        font.drawText(renderer,
                      "SPA: " + std::to_string(sp.baseStats.specialAttack),
                      col2X, statsY, scale);
        font.drawText(renderer,
                      "SPD: " + std::to_string(sp.baseStats.specialDefense),
                      col2X, statsY + 14 * scale, scale);
        font.drawText(renderer, "SPE: " + std::to_string(sp.baseStats.speed),
                      col2X, statsY + 28 * scale, scale);
    } else {
        int statsY = infoY + 16 * scale;
        font.drawText(renderer, "Catch this Daemon to", infoX, statsY, scale);
        font.drawText(renderer, "see more details!", infoX, statsY + 14 * scale,
                      scale);
    }
