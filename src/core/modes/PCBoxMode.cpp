        ...
        if (dir == Direction::left || dir == Direction::right) {
            if (!lrHeld) {
                ...
                if (dir == Direction::left)
                    box = (box + Player::NUM_BOXES - 1) % Player::NUM_BOXES;
                else
        ...
        if (viewingParty) {
            ...
            if (player.partySize() > 0 && selected >= 0 && selected < player.partySize()) {
                if (!player.canDeposit()) {
                    ...
                    if (partySize == 0)
                        selected = 0;
                    else if (selected >= partySize)
                        selected = partySize - 1;
                }
            }
        } else {
            // Withdraw: box → party
            int boxCount = player.getBoxCount(player.getCurrentBox());
            if (boxCount > 0 && selected >= 0 && selected < boxCount) {
