...
void TitleScreenMode::update(GameContext &ctx, InputManager &input) {
    ...
    case Phase::titleCard: {
        ...
        if (titleTimer > 15 &&
            (input.isConfirmPressed() || input.isCancelPressed())) {
    ...
    case Phase::saveSlotSelect: {
        ...
        if (input.isConfirmPressed()) {
            ...
            MusicTrack mapTrack =
                MusicManager::trackForMap(ctx.world.getCurrentMapId());
...
void TitleScreenMode::render(GameContext &ctx) {
    ...
    case Phase::titleCard: {
        ...
        if (titleTimer > 5) {
            ctx.ui.getSpriteFont().drawText(
                r, "CIPHERBOUND", WINDOW_WIDTH / 2 - 11 * 8 * PIXEL_SCALE / 2,
                WINDOW_HEIGHT / 2 - 60, PIXEL_SCALE, 1);

            // Subtitle
            r.drawText("A Science Daemon Adventure", WINDOW_WIDTH / 2 - 110,
                       WINDOW_HEIGHT / 2 + 10, TDT4102::Color{160, 160, 200},
                       14);
        }

        // "Press any key" blink
        if (titleTimer > 15 && (titleTimer / 15) % 2 == 0) {
            r.drawText("Press Z or Enter to continue", WINDOW_WIDTH / 2 - 110,
                       WINDOW_HEIGHT - 60, TDT4102::Color{120, 120, 170}, 12);
        }
        break;
    }
    case Phase::saveSlotSelect: {
        ...
        renderer.drawFilledRect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT,
                                TDT4102::Color{20, 20, 60});

        // Title
        spriteFont.drawText(renderer, "CIPHERBOUND",
                            WINDOW_WIDTH / 2 - 11 * 8 * PIXEL_SCALE / 2, 40,
                            PIXEL_SCALE, 1);
        ...
        for (int i = 0; i < slotCount; ++i) {
            ...
            TDT4102::Color bgColor = isSelected ? TDT4102::Color{60, 60, 120}
            ...
            if (isSelected)
                ctx.ui.drawSelectionArrow(startX + 8,
                                          y + slotBoxH / 2 - PIXEL_SCALE * 3,
                                          PIXEL_SCALE);
            ...
            renderer.drawText(slotLabel, textX, textY, TDT4102::Color::white,
                              18);
            ...
            if (slotInfo.exists) {
                ...
                renderer.drawText(info, textX, textY + 30,
                                  TDT4102::Color::light_gray, 14);
            } else {
                renderer.drawText("New Game", textX, textY + 30,
                                  TDT4102::Color{140, 140, 180}, 14);
            }
        }
