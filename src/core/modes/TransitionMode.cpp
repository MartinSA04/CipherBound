...
void TransitionMode::render(GameContext &ctx) {
    ...
    int transitionAlpha = static_cast<int>(static_cast<float>(ctx.world.getPlayer().getAnimationFrame()) /
                                           static_cast<float>(moveDelay) * 255.0f);
    int alpha = fadeOut ? 255 - transitionAlpha : transitionAlpha;
    r.drawFilledRect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, TDT4102::Color{0, 0, 0, static_cast<unsigned char>(alpha)});
}
