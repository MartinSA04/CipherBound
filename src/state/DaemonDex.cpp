...
bool DaemonDex::hasSeen(int speciesId) const { return seenSpecies.count(speciesId) > 0; }
bool DaemonDex::hasCaught(int speciesId) const { return caughtSpecies.count(speciesId) > 0; }
int DaemonDex::seenCount() const { return static_cast<int>(seenSpecies.size()); }
int DaemonDex::caughtCount() const { return static_cast<int>(caughtSpecies.size()); }
