...
void Party::swapDaemon(int indexA, int indexB) {
    if (indexA >= 0 && indexA < partySize() && indexB >= 0 &&
        indexB < partySize())
        std::swap(party[static_cast<std::size_t>(indexA)],
                  party[static_cast<std::size_t>(indexB)]);
}
...
void PartyAndPCBoxes::depositDaemon(int partyIndex) {
    if (!canDeposit() || partyIndex < 0 || partyIndex >= partySize())
        return;
    if (static_cast<int>(
            pcBoxes[static_cast<std::size_t>(currentBox)].size()) >= BOX_SIZE)
        return;
...
void PartyAndPCBoxes::withdrawDaemon(int boxIndex, int slot) {
    ...
    if (slot < 0 ||
        slot >= static_cast<int>(
                    pcBoxes[static_cast<std::size_t>(boxIndex)].size()))
        return;

    party.push_back(std::move(pcBoxes[static_cast<std::size_t>(boxIndex)]
...
int PartyAndPCBoxes::getBoxCount(int boxIndex) const {
    return static_cast<int>(
        pcBoxes.at(static_cast<std::size_t>(boxIndex)).size());
}
