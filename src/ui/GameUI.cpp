...
void GameUI::drawOverworld(const Map &map, const Player &player, const std::vector<std::shared_ptr<NPC>> &npcs) {
...
void GameUI::drawPartyList(const Player &player, int selected) {
    ...
    for (int i = 0; i < partyCount; ++i) {
        ...
        TDT4102::Color bgColor = isSelected ? TDT4102::Color{200, 215, 255} : TDT4102::Color{230, 235, 250};

        renderer.drawFilledRect(20, sy, WINDOW_WIDTH - 40, slotHeight, bgColor);
        renderer.drawRect(20, sy, WINDOW_WIDTH - 40, slotHeight, TDT4102::Color::transparent, TDT4102::Color::black);
        ...
        spriteFont.drawText(renderer, std::to_string(c.getCurrentHP()) + "-" + std::to_string(c.getMaxHP()), hpBarX,
                            sy + 9 * scale, scale);
...
void GameUI::drawSummaryScreen(const Daemon &daemon, const Pokedex &pokedex, int page) {
    ...
    spriteFont.drawText(renderer, lvlStr, 24 + spriteFont.getTextWidth(daemon.getNickname(), scale) + 8 * scale,
                        headerY, scale);
...
void GameUI::drawBagScreen(const Player &player, const Pokedex &pokedex, int selected) {
    ...
    for (int i = 0; i < inventoryCount; ++i) {
        ...
        TDT4102::Color bgColor = isSelected ? TDT4102::Color{255, 230, 180} : TDT4102::Color{240, 220, 200};
    ...
    if (selected >= 0 && selected < static_cast<int>(inventory.size())) {
        const ItemData &item = pokedex.getItem(inventory[static_cast<std::size_t>(selected)].itemId);
...
bool GameUI::isTextFullyRevealed() const { return typewriterCharsRevealed >= typewriterFullText.size(); }
...
bool GameUI::tickHPAnimation(int targetPlayerHP, int targetOpponentHP, int maxPlayerHP, int maxOpponentHP) {
...
EXPTickResult GameUI::tickEXPAnimation(int targetEXP, int expNeeded) {
    ...
    playerDisplayEXP = expAnimStartEXP + (destination - expAnimStartEXP) * (expAnimFrame / EXP_ANIM_FRAMES);
...
void GameUI::drawDialogueBox(const std::string &speaker, const std::string &text) {
    ...
    spriteFont.drawTextPartial(renderer, text, typewriterCharsRevealed, textX, textY, scale, 1, textMaxW);
    ...
    if (!speaker.empty()) {
        ...
        renderer.drawRect(spkX, spkY, spkW, spkH, TDT4102::Color::transparent, TDT4102::Color::black);
...
void GameUI::drawChoiceBox(const std::vector<std::string> &options, int selected) {
    ...
    renderer.drawRect(boxX, boxY, boxWidth, boxHeight, TDT4102::Color::transparent, TDT4102::Color{60, 70, 100});

    int oy = boxY + 8;
    for (int i = 0; i < optionCount; ++i) {
        if (i == selected)
            drawSelectionArrow(boxX + 2 * scale, oy + 4 * scale, scale);
        spriteFont.drawText(renderer, options[static_cast<std::size_t>(i)], boxX + 6 * scale, oy, scale);
        oy += itemHeight;
    }
}
...
void GameUI::drawNarrowTextBar(int x, int y, int srcW, int scale) {
    ...
    renderer.drawSpriteRegion("ui_text_bar", midSrcX, 0, middleSrcW, srcH, x + edgeW * scale, y, middleSrcW * scale,
                              dstH);

    // Right edge
    renderer.drawSpriteRegion("ui_text_bar", fullSrcW - edgeW, 0, edgeW, srcH, x + (edgeW + middleSrcW) * scale, y,
                              edgeW * scale, dstH);
}
...
void GameUI::drawOpponentInfoBar(const Daemon *opponentDaemon, int offsetX) {
    ...
    if (opponentDaemon) {
        ...
        drawSpriteHPBar(oppHPBarX, oppHPBarY, oppHPBarW, opponentDisplayHP, opponentDaemon->getMaxHP(), scale);
    }
}
void GameUI::drawPlayerInfoBar(const Daemon *playerDaemon, int offsetX) {
    ...
    if (playerDaemon) {
        ...
        drawSpriteHPBar(plyHPBarX, plyHPBarY, plyHPBarW, playerDisplayHP, playerDaemon->getMaxHP(), scale);
...
void GameUI::drawOpponentTrainer(const NPC *opponent, int offsetX) {
    ...
    if (opponent) {
        spriteFont.drawText(renderer, opponent->getName(), trainerX + 2 * scale, trainerY + trainerH / 2, scale - 1);
    }
}
