...
void SpriteFont::drawText(Renderer &renderer, const std::string &text,
                          int screenX, int screenY, int scale, int spacing) const
{
    drawTextPartial(renderer, text, static_cast<int>(text.size()), screenX, screenY, scale, spacing);
}

void SpriteFont::drawTextPartial(Renderer &renderer, const std::string &text, int charCount,
                                 ...
                                 int maxWidth) const
{
    ...
    if (maxWidth > 0)
    {
        ...
        while (i < text.size() && drawn < charCount)
        {
            ...
            int spaceCount = static_cast<int>(textStart - wordStart);
            int spacesWidth = spaceCount * SPACE_WIDTH * scale;
            ...
            // Draw leading spaces
            for (int s = 0; s < spaceCount && drawn < charCount; ++s)
            {
            ...
            for (char c : word)
            {
                ...
                if (glyph)
                {
                    ...
                    renderer.drawSpriteRegion(FONT_TEXTURE,
...
void SpriteFont::drawBattleNumber(Renderer &renderer, int number,
                                  int screenX, int screenY, int scale, bool rightAlign) const
{
...
void SpriteFont::drawBattleNumberString(Renderer &renderer, const std::string &text,
                                        int screenX, int screenY, int scale) const
{
    int cursorX = screenX;
    ...
    for (char c : text)
    {
        if (c >= '0' && c <= '9')
        {
            ...
            renderer.drawSpriteRegion(BNUM_TEXTURE,
                                      srcX, srcY, srcW, srcH,
                                      cursorX, screenY,
                                      dstW, dstH);
            cursorX += dstW;
