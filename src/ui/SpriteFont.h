...
public:
    ...
    void drawText(Renderer &renderer, const std::string &text,
                  int screenX, int screenY, int scale = PIXEL_SCALE,
                  int spacing = 1) const;

    // Draw only the first 'charCount' characters of text (for typewriter effect)
    // If maxWidth > 0, word-wrap text to fit within maxWidth pixels
    void drawTextPartial(Renderer &renderer, const std::string &text, int charCount,
                         int screenX, int screenY, int scale = PIXEL_SCALE,
    ...
    void drawBattleNumber(Renderer &renderer, int number,
                          int screenX, int screenY, int scale = PIXEL_SCALE, bool rightAlign = false) const;
    ...
    void drawBattleNumberString(Renderer &renderer, const std::string &text,
                                int screenX, int screenY, int scale = PIXEL_SCALE) const;
