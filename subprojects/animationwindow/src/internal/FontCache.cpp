...
void TDT4102::internal::FontCache::loadFont(nk_context *context, TDT4102::Font face, unsigned int size) {
    ...
    if(face == Font::defaultFont || fontFileLocations.count(face) == 0) {
...
void TDT4102::internal::FontCache::initialize() {
    ...
    // No system fonts available under Emscripten; all non-default fonts
    // will fall back to the built-in default when requested.
    return;
#else
